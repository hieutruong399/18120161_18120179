...
void SCREEN::DisplayScreen()//Hàm vẽ màn hình ở tất cả các giai đoạn
{
	...
	*/
	...
	case 0://Giai đoạn bắt đầu trò chơi, hiển thị màn hình Menu trò chơi
		...
		//
		break;
	case 1:case 2://2 Giai đoạn này là chế độ 1 và 2 người chơi thì các đối tượng cần hiển thị ra màn hình giống nhau hoàn toàn
	...
	case 3://Giai đoạn đã chơi xong và đã xác định được người thắng
		//Đoạn code dưới đây giống như ở trường hợp case 1 và case 2, bởi vì ta cũng cần hiển thị ra các ôối tượng giống như vậy
		//Tuy nhiên ta không gộp chung vì ta còn cần hiển thị thêm các đối tượng khác
		glClearColor(0, 0.9, 0.5, 0.1);
		glColor3f(0.4, 0.1, 0.0);
		b.Update(0, 1150, TopBar + 5, TopBar);
		b.Draw();
		glColor3f(1.0, 0.1, 0.0);//Đặt màu vẽ là màu đỏ
		s = to_string(LeftRacket.Score);
		PrintText(Width / 2 - 25, TopBar + 15, s);
		PrintText(Width / 2, TopBar + 15, ":");
		s = to_string(RightRacket.Score);
		PrintText(Width / 2 + 20, TopBar + 15, s);
...
void SCREEN::SetUpDefaultStartValue()//Hàm khởi tạo các giá trị ban đầu
{
	...
	LeftRacket.Top = 400;
	LeftRacket.Bottom = 250;
	LeftRacket.Speed = 0.5;
	LeftRacket.Status = 1;
	LeftRacket.Score = 0;
	//
	RightRacket.Left = 1070;
	RightRacket.Right = 1100;
	RightRacket.Top = 400;
	RightRacket.Bottom = 250;
	RightRacket.Speed = 0.5;
	RightRacket.Status = 1;
	RightRacket.Score = 0;
}
SCREEN::SCREEN()
